...
calCindex <- function(model, name,
  ...
  CohortSurv[["Test"]] <- Test_surv[,c("Sample", CohortVar, timeVar, statusVar)]
   ...
   CohortSet[["ALL"]] <- ALL_expr
  ALL_surv$Sample <- rownames(ALL_surv)
  CohortSurv[["ALL"]] <- ALL_surv[,c("Sample", CohortVar, timeVar, statusVar)]
  ...
  if(metaCohort){
    CohortSet[["Meta"]] <- ALL_expr
    CohortSurv[["Meta"]] <- ALL_surv

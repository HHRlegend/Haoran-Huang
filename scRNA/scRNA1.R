...
samples=list.files('GSE242889_RAW/')
samples
sceList = lapply(samples,function(pro){ 
    folder=file.path('GSE242889_RAW/',pro) 
    print(pro)
    print(folder)
...
sceList$'1T'@meta.data$group <- 'MVI_positive'
sceList$'2T'@meta.data$group <- 'MVI_positive'
sceList$'3T'@meta.data$group <- 'MVI_positive'
sceList$'4T'@meta.data$group<- 'MVI_negative'
sceList$'5T'@meta.data$group<- 'MVI_negative'
...
sce.all.filt<- FindNeighbors(sce.all.filt, dims = 1:dim.usage) %>%
  ...
  sweep.res.list <- paramSweep_v3(sce.all.filt, PCs = 1:dim.usage, sct = FALSE) 
...
save(sce.all.filt,file = 'sce.all.filt1.Rdata') 
